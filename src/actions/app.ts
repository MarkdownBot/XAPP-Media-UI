...
export function getApps(): (dispatch: Dispatch<State>) => Promise<GetAppsSuccessAction> {
  return (dispatch: Dispatch<State>): Promise<GetAppsSuccessAction> => {
    ...
    return fetch('https://api.xapp.media/apps?organizationId=FederatedMedia&limit=500', {
      ...
      document.getElementById('errorLoadingApps').innerHTML = "Loaded";
      return body.json();
    }).then((json) => {
      document.getElementById('errorLoadingApps').innerHTML = "";
      document.getElementById('errorLoadingApps').className = "";
      return dispatch(getAppsSuccess(json.apps));
  }).catch(err => {
      document.getElementById('errorLoadingApps').innerHTML = "Error Loading Apps";
      document.getElementById('errorLoadingApps').className = "alert alert-danger";
      return err.json();
  });
  };
}
